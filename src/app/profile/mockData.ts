...
export const mockCurrentProject: CurrentProject = {
  ...
  description: "Building an intelligent resume parsing system with Python and spaCy",
...
const generateContributionGrid = (): ContributionGrid => {
  const data: ContributionLevel[] = Array.from({ length: 30 * 7 }, () => 
    Math.floor(Math.random() * 5) as ContributionLevel
  );
  
  const totalContributions = data.reduce<number>((sum, level) => sum + level, 0);
...
export const mockContributionGrid: ContributionGrid = generateContributionGrid();
