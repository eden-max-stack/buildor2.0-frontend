...
import { 
  User, MapPin, Link as LinkIcon, Github, Settings, 
  BookOpen, Trophy, Code, ExternalLink,
  Search, Filter, Clock, Cpu, Award, GraduationCap,
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
            ...
            <div className="md:col-span-4 lg:col-span-3 space-y-6">
              <div className="flex flex-col items-start">
                ...
                <h1 className="text-3xl font-bold text-gray-900 dark:text-white">{leftProfileCard?.fullName || "Profile"}</h1>
                <p className="text-xl text-gray-500 dark:text-gray-400 mb-4">@{leftProfileCard?.username}</p>
                ...
                <div className="space-y-2 text-sm text-gray-600 dark:text-gray-400 w-full">
                  ...
                  {leftProfileCard?.website && (
                    <div className="flex items-center gap-2">
                      ...
                      <a href={leftProfileCard?.website} className="hover:text-blue-500 truncate">
                        {leftProfileCard?.website}
                      </a>
                    </div>
                  )}
              ...
              <div>
                <h3 className="font-semibold text-gray-900 dark:text-white mb-3">Top Skills</h3>
                <div className="flex flex-wrap gap-2">
                  {leftProfileCard?.skills?.map((skill) => (
                    <span key={skill} className="px-3 py-1 text-xs font-medium rounded-full bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-200 border border-blue-200 dark:border-blue-800">
                      {skill}
                    </span>
                  ))}
                </div>
            ...
            <div className="md:col-span-8 lg:col-span-9">
              ...
              <div className="border-b border-gray-200 dark:border-gray-700 mb-6 overflow-x-auto no-scrollbar">
                <nav className="flex space-x-8">
                  {(['Overview', 'Questions Solved', 'Portfolio'] as TabName[]).map((tab) => (
                    <button 
                      key={tab}
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <section className="mb-8">
        <div className="flex items-center justify-between mb-2">
          ...
          <button className="text-xs text-blue-600 dark:text-blue-400 hover:underline">Customize</button>
        </div>
        ...
        <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-6 bg-white dark:bg-gray-900">
            ...
            <article className="prose dark:prose-invert max-w-none">
              <h3 className="text-xl font-bold text-gray-800 dark:text-gray-100">Hi there! 👋</h3>
              <p className="text-gray-600 dark:text-gray-300 text-sm mt-2">
                I'm a passionate developer currently solving complex algorithmic challenges. 
                I'm working on innovative projects to grow my skills.
              </p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-6">
                  {currentProject && (
                    <div className="p-4 rounded border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800">
                      ...
                      <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">{currentProject.description}</p>
                    </div>
                  )}
                  {recentAchievement && (
                    <div className="p-4 rounded border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800">
                      <h4 className="font-semibold text-sm text-gray-900 dark:text-white flex items-center gap-2">
                          <Trophy className="w-4 h-4 text-yellow-500" /> Recent Achievement
                      </h4>
                      <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">{recentAchievement.title}</p>
                    </div>
                  )}
              </div>
            </article>
      ...
      <section className="mb-8">
        <h2 className="text-base font-semibold text-gray-900 dark:text-white mb-2">
          {contributionGrid.totalContributions.toLocaleString()} contributions in the last year
        </h2>
        <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-4 bg-white dark:bg-gray-900 overflow-x-auto">
          <div className="min-w-[700px]">
            ...
            <div className="flex justify-between items-center mt-4 text-xs text-gray-500 dark:text-gray-400">
                <span>Learn more about how contribution counts are calculated</span>
                <div className="flex items-center gap-1">
                  <span>Less</span>
                  {activityLevels.map((className, i) => (
                    <div key={i} className={`w-3 h-3 ${className} rounded-sm`}></div>
                  ))}
                  <span>More</span>
                </div>
            </div>
      ...
      <section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {solvedQuestions.map((question) => (
              <div key={question.id} className="flex flex-col justify-between p-4 border border-gray-200 dark:border-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors bg-white dark:bg-gray-900">
                <div className="flex items-start justify-between mb-2">
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded-full ${getDifficultyColorClass(question.difficulty)}`}>
                      {question.difficulty}
                    </span>
                </div>
                <div className="flex items-center gap-3 text-xs text-gray-500 dark:text-gray-400 mt-2">
                    <span className="flex items-center gap-1">
                      <div className={`w-2 h-2 rounded-full ${getLanguageColor(question.lang)}`}></div> 
                      {question.lang}
                    </span>
  ...
  return (
    <div className="space-y-6 animate-in slide-in-from-right-4 duration-300">
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center">
         <h2 className="text-lg font-bold text-gray-900 dark:text-white">Solving History</h2>
      ...
      <div className="space-y-4">
        {questions.map((q) => (
          <div key={q.id} className="border border-gray-200 dark:border-gray-700 rounded-lg p-5 bg-white dark:bg-gray-900 hover:shadow-md transition-all">
            
            {/* Top Row: Title, Difficulty, Language */}
            <div className="flex flex-wrap justify-between items-start gap-4 mb-4">
              <div className="flex items-start gap-3">
                 <div className={`mt-1 p-2 rounded-lg ${getDifficultyBgClass(q.difficulty)}`}>
                    <Code className={`w-5 h-5 ${getDifficultyTextClass(q.difficulty)}`} />
                 </div>
                 <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                       <span className={`text-xs px-2 py-0.5 rounded-full border ${getDifficultyColorClass(q.difficulty)}`}>
                          {q.difficulty}
                       </span>
              ...
              <div className="text-right">
                 <span className="text-sm font-semibold text-gray-900 dark:text-white">{q.lang}</span>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
               <div className="flex items-center gap-2 text-gray-600 dark:text-gray-400">
                  ...
                  <span className="font-medium text-gray-900 dark:text-gray-200">{q.runtime}</span>
                  <span className="text-xs">Runtime</span>
               </div>
               <div className="flex items-center gap-2 text-gray-600 dark:text-gray-400">
                  <Cpu className="w-4 h-4 text-purple-500" />
                  <span className="font-medium text-gray-900 dark:text-gray-200">{q.memory}</span>
                  <span className="text-xs">Memory</span>
               </div>
               <div className="flex items-center gap-2 text-gray-600 dark:text-gray-400">
                  <Trophy className="w-4 h-4 text-yellow-500" />
                  <span className="font-medium text-gray-900 dark:text-gray-200">{q.rank}</span>
                  <span className="text-xs">Ranking</span>
               </div>
               <div className="flex items-center gap-2 text-gray-600 dark:text-gray-400">
                  <div className={`w-4 h-4 rounded-full flex items-center justify-center font-bold text-[10px] ${q.hintsUsed > 0 ? 'bg-orange-100 text-orange-600' : 'bg-gray-100 text-gray-400'}`}>
                    ?
                  </div>
                  <span className={`font-medium ${q.hintsUsed > 0 ? 'text-orange-600' : 'text-gray-900 dark:text-gray-200'}`}>
                    {q.hintsUsed}
                  </span>
      ...
      <div className="lg:col-span-2 space-y-8">
        ...
        <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-6 bg-white dark:bg-gray-900">
            ...
            <article className="prose dark:prose-invert max-w-none">
              ...
              <p className="text-gray-600 dark:text-gray-300 text-sm mt-2">
                I'm a passionate developer currently solving complex algorithmic challenges. 
                I'm working on innovative projects to sharpen my problem-solving skills.
              </p>
        ...
        <section>
          <div className="flex items-center gap-2 mb-3">
             ...
             <h2 className="text-lg font-bold text-gray-900 dark:text-white">Professor Feedback</h2>
          </div>
          <div className="space-y-4">
             {professorFeedback.map((fb) => (
                <div key={fb.id} className="border border-gray-200 dark:border-gray-700 rounded-lg p-5 bg-white dark:bg-gray-900 relative">
                   <div className="flex justify-between items-start mb-2">
                      <div className="flex items-center gap-3">
                         <div className="w-10 h-10 rounded-full bg-blue-100 dark:bg-blue-900 flex items-center justify-center text-blue-700 dark:text-blue-300 font-bold text-sm">
                           {fb.prof.split(' ').map(n => n[0]).join('')}
                         </div>
                         <div>
                            <h4 className="font-semibold text-gray-900 dark:text-white text-sm">{fb.prof}</h4>
                   ...
                   <div className="pl-12">
                      <p className="text-gray-600 dark:text-gray-300 text-sm italic">"{fb.comment}"</p>
                   </div>
                </div>
             ))}
          </div>
          ...
          <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-5 bg-white dark:bg-gray-900">
             ...
             <div className="space-y-3 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-gray-900 dark:text-white">{academicInfo.university}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Degree</span>
                  <span className="font-medium text-gray-900 dark:text-white">{academicInfo.degree}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">GPA</span>
                  <span className="font-medium text-green-600">{academicInfo.gpa}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Expected Grad</span>
                  <span className="font-medium text-gray-900 dark:text-white">{academicInfo.expectedGraduation}</span>
                </div>
             </div>
          </div>
        )}
          ...
          <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-5 bg-white dark:bg-gray-900">
             ...
             <div className="space-y-3">
                {externalLinks.map((link) => (
                  <a key={link.id} href={link.url} className="flex items-center justify-between p-2 hover:bg-gray-50 dark:hover:bg-gray-800 rounded transition-colors group">
