...
import { useState, useEffect, useRef, type RefObject, use } from "react";
...
export default function CodeSandbox({
  ...
  useEffect(() => {
    const fetchQuestion = async () => {
      ...
      try {
        setIsLoadingQuestion(true); // Ensure loading state is set
        ...
        // Set both states so your UI works perfectly
        setQuestion(data);
        setQuestionData(data); // In case you use this elsewhere

        // Set initial code template using the first test case
        ...
        setLoading(false); // Clear the top-level loading state too
      }
    };
  ...
  // Remove the old fetchQuestionDetails function completely!
  ...
  const renderAST = (astData: any, svgRef: RefObject<SVGSVGElement | null>) => {
    ...
    // --- NEW: Convert flat nodes/edges to hierarchical structure ---
    let rootNodeData;

    // Check if the backend gave us a pre-built root tree or a flat list
    if (astData.root && astData.root.children !== undefined) {
      // It's already a tree
      rootNodeData = astData.root;
    } else if (astData.nodes && astData.edges) {
      // It's a flat graph, we need to build the tree
      const nodeMap = new Map();

      // 1. Initialize map with all nodes
      astData.nodes.forEach((node: any) => {
        nodeMap.set(node.id, { ...node, children: [] });
      });

      // 2. Attach children using edges
      astData.edges.forEach((edge: any) => {
      ...
      // 3. Find the root (a node with no incoming edges)
      const targetIds = new Set(astData.edges.map((e: any) => e.target));
      const roots = astData.nodes.filter((n: any) => !targetIds.has(n.id));

      if (roots.length > 0) {
        rootNodeData = nodeMap.get(roots[0].id);
      } else {
        // Fallback if graph is weird
        rootNodeData = nodeMap.values().next().value;
      }
    }

    if (!rootNodeData) {
      console.error("Could not construct AST hierarchy from data:", astData);
      return;
    }
    // ---------------------------------------------------------------

    // Convert AST to d3 hierarchy using our safely constructed root
    const root = d3.hierarchy(rootNodeData, (d: any) => d.children || []);
