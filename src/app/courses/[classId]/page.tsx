...
export default function BrowseClassDetail() {
  ...
  const [trainerProfile, setTrainerProfile] = useState<Record<
    string,
    any
  > | null>(null);
  ...
  const loadData = async () => {
    try {
      // 1. Get the session directly here so we have access to the user ID
      const {
        data: { session },
      } = await supabase.auth.getSession();
      const headers: Record<string, string> = {
      ...
      const [detailRes, materialsRes, quizzesRes, enrollRes] =
        await Promise.all([
          ...
          fetch(`${API_BASE}/api/classes/${classId}/enrollment-status`, {
            headers,
          }), // It's okay if this fails now
        ]);

      let isTrainer = false; // Define this flag outside

      if (detailRes.ok) {
        const detail = await detailRes.json();
        setClassDetail(detail);

        // 2. FOOLPROOF CHECK: Is the logged in user the trainer of this class?
        if (session?.user?.id) {
          isTrainer = session.user.id === detail.trainer_id;
        }
        ...
        if (detail.trainer_id) {
          try {
            const trainerRes = await fetch(
              `${API_BASE}/api/profiles/${detail.trainer_id}`,
              { headers },
            );
            if (trainerRes.ok) {
      ...
      if (enrollRes.ok) {
        ...
        // 3. Merge backend enrollment data with our foolproof trainer check
        setEnrollment({
          is_enrolled: enrollData.is_enrolled,
          is_trainer: isTrainer,
          enrolled_at: enrollData.enrolled_at || null,
        });
      } else {
        // 5. FALLBACK: If the enrollment endpoint 404s, at least set the trainer status!
        setEnrollment({
          is_enrolled: false,
          is_trainer: isTrainer, // This is the crucial part that fixes your UI!
          enrolled_at: null,
        });
      }
    } catch (err) {
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-[#0a0f1c] text-brand-dark dark:text-gray-200 py-8 transition-colors">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
          ...
          <div className="relative bg-gradient-to-r from-brand-dark to-blue-900 dark:from-gray-900 dark:to-gray-800 rounded-2xl p-8 overflow-hidden shadow-xl border border-gray-800">
            ...
            <div className="relative z-10 flex flex-col md:flex-row md:items-end justify-between gap-8">
              ...
              <div className="shrink-0">
                {enrollment?.is_trainer ? (
                  <button
                    onClick={() => router.push(`/courses/${classId}/edit`)} // Assuming this is your edit route
                    className="flex items-center gap-2 px-6 py-3 bg-gray-800 hover:bg-gray-700 text-white rounded-xl font-medium transition-colors shadow-lg border border-gray-700"
                  >
                    <Settings className="w-5 h-5" />
                    Edit Class
                  </button>
                ) : enrollment?.is_enrolled ? (
                  <button className="flex items-center gap-2 px-6 py-3 bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl font-medium transition-colors shadow-lg">
          ...
          <div className="min-h-[400px]">
                            ...
                            >
                              ...
                              {canAccess && (
                                <Link
                                  href={
                                    isQuestion
                                      ? `/sandbox/${item.question_id}`
                                      : `/courses/${classId}/material/${item.material_id}`
                                  }
                                  className="shrink-0 flex items-center gap-1 text-sm font-semibold text-gray-400 group-hover:text-brand-blue transition-colors"
                                >
                                  {isQuestion
                                    ? "Solve"
                                    : isVideo
                  ...
                  quizzes.map((quiz) => (
                    ...
                    >
                      <div className="px-6 py-4 bg-gray-50 dark:bg-gray-800/80 border-b border-gray-200 dark:border-gray-700/50 flex items-center justify-between">
                        ...
                        <span>
                          <Link
                            href={`/courses/${classId}/quiz/${quiz.quiz_id}`}
                          >
                            <ChevronRight />
                          </Link>
                        </span>
            ...
            {/* STUDENTS TAB (Only visible to the trainer) */}
            {activeTab === "students" && enrollment?.is_trainer && (
                  ...
                  {students.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {students.map((student, idx) => (
                        <div
                          key={idx}
                          className="flex items-center gap-4 p-4 border border-gray-100 dark:border-gray-700/50 rounded-xl bg-gray-50 dark:bg-gray-800/50 hover:border-brand-blue/30 transition-colors"
                        >
                          <div className="w-12 h-12 rounded-full bg-gradient-to-br from-brand-blue to-blue-400 flex items-center justify-center text-white font-bold shrink-0">
              ...
              <div className="max-w-2xl mx-auto bg-white dark:bg-gray-800 rounded-2xl p-4 border border-gray-200 dark:border-gray-700 shadow-2xl flex items-center justify-between gap-4">
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    {phases.length} phases · {totalMaterials} materials ·{" "}
